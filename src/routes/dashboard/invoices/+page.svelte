<script lang="ts">
	...
	const proformas = $derived(data.proformas || []);
	...
	// - Total facturado = sum of factura total_amount (invoices already issued).
	// - Monto pagado = sum of paid facturas total_amount.
	// - Pendiente: for each client, proforma total - factura total (clamped to 0).
	//   Each factura is progress against the proforma contract; pendiente is the
	//   remaining un-invoiced portion of the contract.
	...
	const totalPending = $derived.by(() => {
		const proformaByClient = totalByClient(proformas);
		const facturaByClient = totalByClient(invoices);
		let total = 0;
...
<svelte:head>
	<title>Facturas - magikalInvoice</title>
</svelte:head>
	...
	>
		<div>
			...
			<p class="mt-0.5 text-xs text-[#707070]">
				Seguimiento, análisis y gestión de las facturas emitidas.
			</p>
		</div>
